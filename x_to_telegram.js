 ...
 *
 * Requirements:
 *   npm install twitter-api-v2 axios dotenv yargs
 *
...
// Use a persistent agent for keep-alive
...
async function sendToTelegram(botToken, chatId, message, disablePreview = false, dryRun = false) {
    if (dryRun) {
    ...
    for (const part of chunks) {
        try {
            ...
            err(`[telegram] HTTP ${e.response?.status || 'Error'} ${e.response?.data?.description || e.message}`);
            throw e;
        }
    }
}
...
async function runOnce(args) {
    ...
    try {
        ...
        for (const t of tweets) {
            ...
            await sendToTelegram(tgToken, tgChat, msg, disablePreview, dryRun);
            lastId = t.id;
